...
get_terrain_metrics <- function(z, w=1) {
  ...
  # If p and q are both zero then the result is 0/0, force to 0 since the
  # surface is flat in this case.
  # if (p == 0 & q == 0) {
  #   return(list(
  #     slope=slope, h_curv=0, v_curv=0
  #   ))
  # }
...
render_evans_young_fit <- function(z, w=1) {
  ...
  z_surf <- matrix(nrow=length(x), ncol=length(y))
  
  # This can be made much more efficient
  for (i in 1:length(x)) {
    for (j in 1:length(y)) {
      # Note flip of j and i here since y is our "row" on the surface.
      z_surf[j, i] <- r*x[i]^2/2 + t*y[j]^2/2 + s*x[i]*y[j] + p*x[i] + q*y[j] + z[5]
    }
  }
